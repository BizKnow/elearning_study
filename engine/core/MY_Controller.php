...
class MY_Controller extends MX_Controller
{
    ...
    public function gen_roll_no($center_id = 1)
    {
        ...
        if ($center_id) {
            ...
            if ($preFixRoll) {
                $lastRollNO = $this->db->select('enrollment_no')->order_by('id', 'DESC')->limit(1)->get('student_courses');
                if ($lastRollNO->num_rows()) {
                    $lastNumber = $lastRollNO->row('enrollment_no');
    ...
    {
        $check = $this->db->select('enrollment_no')->where('enrollment_no', $rollNo)->get('student_courses');
        if ($check->num_rows()) {
            $lastNumber = $check->row('enrollment_no');
            $filterRoll = substr($lastNumber, strlen($preFixRoll));
            if ($filterRoll)
                $start = ($filterRoll + 1);
            return $this->check_roll_exists_or_not($preFixRoll, $preFixRoll . $start);
