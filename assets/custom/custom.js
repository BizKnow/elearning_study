...
const optionsAjaxStudents = (item) => {
    ...
    var template = `<div class="d-flex align-items-center">
                        ...
                        <div class="d-flex flex-column">   
                            ...
                            <span class=" fs-5 text-capitalize "> ${item.admission_no}</span>
...
const label = (html = '', my_class = 'info') => {
    return `<label class="label label-${my_class}">${html}</label>`;
}
const currentYear = () => {
...
const generate_link_btn = (id, type) => {
    return `<button data-id="${id}" data-type="${type}" class="btn click-to-view-link btn-primary pulse btn-sm">
...
$(document).ready(function () {
    // var currentPath = base_url+'/'+window.location.pathname.split("/").pop();
    var currentUrl = window.location.href.split('?')[0];
    // console.log(currentUrl);

    $('.side-nav-link').each(function () {

        if (!$(this).data('bs-toggle') && $(this).attr('href') === currentUrl) {
            // console.log(this);
            $(this).addClass('active');
            $(this).parent().addClass('active');
            $(this).closest('.collapse').addClass('show');
            $(this).closest('ul').closest('.side-nav-item').addClass('active');
        }
    })
...
if (exta_setting_form) {
    ...
    add_new_fields.on("click", function () {
        ...
        $(field_area).append(`<div class="form-group position-relative mb-4">
                                    ...
                                    <a href="javascript:;" class="btn border-1 border-danger border btn-danger h-25px lh-0 w-100 br-none p-2"><i class="ki-outline ki-trash"></i> Delete</a>
        ...
        'columnDefs': [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    button_html += `<a href="${base_url}student/profile/${student_id}" target="_blank" class="btn btn-primary btn-sm">
                                        <i class="fa fa-eye"></i> View
                                    </a>`;
                    if (admission_status != 'all') {
                        if (admission_status == 1) {
                            ...
                            <button class="btn btn-warning btn-sm pending change-status">
                                    <i class="fa fa-arrow-circle-left"></i> Pending
                            </button>
                            */
                            button_html += `
                            <button class="btn btn-danger btn-sm cancel change-status">
                                <i class="fa fa-trash"></i> Cancel
                            </button>`;
                        }
                        else if (admission_status == 2) {
                            button_html += `
                                <button class="btn btn-success btn-sm approve change-status">
                                    <i class="fa fa-check"></i> Approve
                                </button>
                                <button class="btn btn-warning btn-sm pending change-status">
                                    <i class="fa fa-arrow-circle-left"></i> Pending
                                </button>                                
                                <button class="btn btn-danger btn-sm delete change-status">
                                    <i class="fa fa-trash"></i> Cancel
                                </button>`;
                        }
                        else {
...
$(document).on('click', '.advanced-set-page', function () {
    ...
    body.html('');
    if ($pages.length) {
        body.append(`<form data-kt-search-element="form" class="w-100 position-relative mb-2" autocomplete="off" data-gtm-form-interact-id="0">
    ...
    body.append(`<div data-kt-search-element="empty" class="text-center fade">
                    <!--begin::Message-->
    ...
    body.find('[data-kt-search-element="form"] input').on('input', function () {
        ...
        if (!hasResults)
            body.find('[data-kt-search-element="empty"]').removeClass('fade');
        else
            body.find('[data-kt-search-element="empty"]').addClass('fade');
    })
